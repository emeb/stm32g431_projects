...
void Shared_SPI_Blocking_PIO_WriteBytes(uint8_t *pData, uint32_t Size)
{
	...
	while(Size > 0)
	{
		if(Size != 0x1)
		{
			...
			SpiHandle.Instance->DR = *((uint16_t*)pData);
...
{
	// turn on DMA clock
	SPI_DMAMUX_CLK_ENABLE();
	SPI_DMA_CLK_ENABLE();

    // Common
	hdma_spi.Instance                 = SPI_DMA_CHANNEL;
    hdma_spi.Init.Request             = DMA_REQUEST_SPI2_TX;
...
void Shared_SPI_Init(void)
{
	...
	/* Enable NSS pin for AF output */
	//GPIO_InitStructure.Pin =  SPI_NSS_PIN;
	//GPIO_InitStructure.Alternate = SPI_NSS_AF;
	//HAL_GPIO_Init(SPI_NSS_GPIO_PORT, &GPIO_InitStructure);
